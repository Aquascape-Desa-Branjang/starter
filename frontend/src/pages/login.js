  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const userName = decodedToken.name || "Guest"; // Use "Guest" if name is not available

        // Store token, userName, and userRole in localStorage
        localStorage.setItem("authToken", data.token);
        localStorage.setItem("userName", userName);
        localStorage.setItem("userRole", decodedToken.role);
        localStorage.setItem("userPhoto", decodedToken.photo);
      ...
      <div className="w-full md:w-2/3 flex flex-col justify-center items-center bg-gray-100 py-8 md:py-0">
        <div className="w-11/12 md:w-3/4">
          ...
          <form onSubmit={handleLogin}>
            ...
            <button type="submit" className="w-full bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600 transition">Login</button>
            {errorMessage && (
