...
const Dummydata = () => {
    const [pyranometerData, setPyranometerData] = useState('');
    ...
    const handleSubmit2 = async (e) => {
        ...
        const pyr = { radiasi_matahari: pyranometerData}

        try {
            const response = await fetch("http://localhost:5000/api/pyranometer", {
                method: "POST",
                body: JSON.stringify(pyr),
                headers: {
                    "Content-Type": "application/json",
                }
            });
            ...
            <div className="flex-1 p-6">
                ...
                <div className="bg-white shadow-lg rounded-lg border border-gray-300 p-6 mb-6">
                    <table className="min-w-full table-auto mb-6">
                        ...
                        <tbody>
                        <tr>
                            <td className="px-4 py-2 text-sm text-gray-800">Pyranometer</td>
                            <td className="px-4 py-2 text-sm text-gray-800">
                                <form onSubmit={handleSubmit2}>
                                    <input
                                        type="text"
                                        value={pyranometerData}
                                        onChange={(e) => setPyranometerData(e.target.value)}
                                        className="w-3/4 h-10 px-4 py-1 border border-gray-300 rounded-lg"
                                        placeholder="Enter Value"
                                        required
                                    />
